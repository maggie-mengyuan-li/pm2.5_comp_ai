...
# Join county lat lon data with soil
soil_lat_lon = left_join(soil, count_centroid)
...
```{r cache = TRUE}
# soil_tensor_mod_10 =  gamm4(soil ~ county_type + as.factor(year) + popd_q + hhinc_q
#                         + t2(lat, long,
...
#                  data = soil_lat_lon) 
# 
# summary(soil_tensor_mod_10$gam)
# summary(soil_tensor_mod_10$mer)
# soil_tensor_mod_10$gam
# 
# ## save model output as RDS to read back in later
# saveRDS(soil_tensor_mod_10,
#         file = "../intermediate/model_outputs/spatial/main_effects_only/soil_tensor_10knots.rds")
soil_tensor_mod_10 = readRDS("../intermediate/model_outputs/spatial/main_effects_only/soil_tensor_10knots.rds")
soil_tensor_mod_10
summary(soil_tensor_mod_10$mer)
summary(soil_tensor_mod_10$mer)$coefficients[2,2]

## Extract 95% CI: point estimate, LL, UL
paste(summary(soil_tensor_mod_10$mer)$coefficients[2,1] %>% round(digits = 3),
      " (",
      round(summary(soil_tensor_mod_10$mer)$coefficients[2,1] - 1.96*summary(soil_tensor_mod_10$mer)$coefficients[2,2], digits = 3),
      ", ",
      round(summary(soil_tensor_mod_10$mer)$coefficients[2,1] + 1.96*summary(soil_tensor_mod_10$mer)$coefficients[2,2], digits = 3),
      ")", sep = "")
paste(summary(soil_tensor_mod_10$mer)$coefficients[2,1] %>% round(digits = 2),
      " (",
      round(summary(soil_tensor_mod_10$mer)$coefficients[2,1] - 1.96*summary(soil_tensor_mod_10$mer)$coefficients[2,2], digits = 2),
      ", ",
      round(summary(soil_tensor_mod_10$mer)$coefficients[2,1] + 1.96*summary(soil_tensor_mod_10$mer)$coefficients[2,2], digits = 2),
      ")", sep = "")
```
...
#                              bs = 'cr'),
#                  random = ~(1|State/GEOID),
#                  data = soil_lat_lon) # try default 10 knots
# 
# summary(soil_tensor_mod_10$gam)
# summary(soil_tensor_mod_10$mer)
# soil_tensor_mod_10$gam
...
# # save model output as RDS to read back in later
# saveRDS(soil_tensor_mod_10,
#         file = "../intermediate/model_outputs/spatial/soil_tensor_10knots.rds")
soil_tensor_mod_10 = readRDS("../intermediate/model_outputs/spatial/soil_tensor_10knots.rds")
soil_tensor_mod_10
summary(soil_tensor_mod_10$gam)
```
...
```{r}
# vcov matrix
native_yr_vcov <- vcov(soil_tensor_mod_10$gam)[c(2,seq(38,54)), c(2,seq(38,54))]
...
soil_decline_model_all <- data.frame()
soil_decline_model_all[1,1] <- summary(soil_tensor_mod_10$gam)$p.coeff[2] # county type main effect estimate (baseline year)
soil_decline_model_all[1,2] <- summary(soil_tensor_mod_10$gam)$p.coeff[2] - 1.96*sd_vector[1] # LL 95% CI
soil_decline_model_all[1,3] <- summary(soil_tensor_mod_10$gam)$p.coeff[2] + 1.96*sd_vector[1] # UL 95% CI

# fill in matrix thru loop for every following year
yr_ct <- 38
for (i in 2:18){
  soil_decline_model_all[i,1] <- summary(soil_tensor_mod_10$gam)$p.coeff[2]+
    summary(soil_tensor_mod_10$gam)$p.coeff[yr_ct]
  soil_decline_model_all[i,2] <- summary(soil_tensor_mod_10$gam)$p.coeff[2]+
    summary(soil_tensor_mod_10$gam)$p.coeff[yr_ct] - 1.96*sd_vector[i]
  soil_decline_model_all[i,3] <- summary(soil_tensor_mod_10$gam)$p.coeff[2]+
    summary(soil_tensor_mod_10$gam)$p.coeff[yr_ct] + 1.96*sd_vector[i]
  yr_ct <- yr_ct + 1
}
# set col names
colnames(soil_decline_model_all) <- c('estimate', 'cl_lower', 'cl_upper')
soil_decline_model_all$year <- seq(2000, 2017)

#PLOT OF TOTAL EFFECT OF NATIVE OVER TIME
modelsp_interx_plot <-ggplot() + 
  theme_linedraw() + 
  geom_line(data = soil_decline_model_all,
  ...
  ylab(expression(paste("Mean Difference in Mineral Dust (", mu, "g/", m^3, ")"))) +
  xlab("Year") +
...
modelsp_interx_plot
ggsave("../figures/effectmod_plots/sensitivity/spatial/soil_interx_t2_10knots.png")
```
